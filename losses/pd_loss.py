...
class MumfordShah_Loss(nn.Module):
    ...
    def levelsetLoss(self, output, target, penalty='l1'):
        ...
        loss = 0.0
        channel_loss = 0.0
        for ich in range(target.shape[1]):  # 遍历每个通道
            target_ = target[:, ich, :, :].unsqueeze(1)
            if output.shape[1] == 1:
                # 如果输出只有一个通道，直接使用该通道
                out_ = output
            else:
                # 如果输出有多个通道，选择相应的通道
                out_ = output[:, ich, :, :].unsqueeze(1)

            pcentroid = torch.sum(target_ * out_, (2, 3)) / torch.sum(out_, (2, 3))
            pcentroid = pcentroid.view(target.shape[0], 1, 1, 1)
            plevel = target_ - pcentroid.expand(target.shape[0], 1, target.shape[2], target.shape[3])
            pLoss = plevel * plevel * out_
            channel_loss += torch.sum(pLoss)

        loss += channel_loss / target.shape[1]  # 平均每个通道的损失
        return loss
