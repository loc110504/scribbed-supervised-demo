...
# from Slot import SoftPositionEmbed,spatial_broadcast,spatial_flatten,spatial_broadcast2,unstack_and_split
from Networks.Slot import SoftPositionEmbed,spatial_broadcast,spatial_flatten,spatial_broadcast2,unstack_and_split
...
class GPM_thin(nn.Module):
    def __init__(self, dilation_series=[6, 12, 18], padding_series=[6, 12, 18], depth=128):
        ...
        super(GPM_thin, self).__init__()
        self.branch_main = nn.Sequential(
            nn.AdaptiveAvgPool2d((1, 1)),
            BasicConv2d(256, depth, kernel_size=1, stride=1)
        )
        self.branch0 = BasicConv2d(256, depth, kernel_size=1, stride=1)
        self.branch1 = BasicConv2d(256, depth, kernel_size=3, stride=1, padding=padding_series[0],
                                   dilation=dilation_series[0])
        self.branch2 = BasicConv2d(256, depth, kernel_size=3, stride=1, padding=padding_series[1],
                                   dilation=dilation_series[1])
        self.branch3 = BasicConv2d(256, depth, kernel_size=3, stride=1, padding=padding_series[2],
...
class ETM(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv_cat = BasicConv2d(4 * out_channels, out_channels * 2, 3, padding=1)
        self.conv_res = BasicConv2d(in_channels, out_channels*2, 1)
...
class ODE(nn.Module):
    def __init__(self, in_channels,num_slots_M,iters,resolutions):
        super(ODE, self).__init__()
        self.SA = SlotAttentionModule(in_channels, resolutions, num_slots_M, iters)
        # self.getalpha = getAlpha(in_channels)

    def forward(self, feature_map):
        f1 = self.SA(feature_map)
        # alpha = self.getalpha(torch.cat([f1,f1],dim=1))
        # out = feature_map+f1*alpha+f1*(1-alpha)
        out = f1 + feature_map
        return  out
    
class ODE_aux(nn.Module):
    def __init__(self, in_channels,num_slots_N,num_slots_M,iters,resolutions):
        super(ODE_aux, self).__init__()
...
class GCM_interFA_ODE_slot_channel4_scribble(nn.Module):
    def __init__(self, in_channels, out_channels, params):
        ...
        self.T1 = ETM(in_channels, int(in_channels/2))
        self.T2 = ETM(in_channels * 2, in_channels)
        self.T3 = ETM(in_channels * 4, in_channels * 2)
        self.T4 = ETM(in_channels * 8, in_channels * 4)
        self.ODE1 = ODE(in_channels * 8, num_slots_M=4,iters=3,resolutions=16)
        self.ODE2 = ODE(in_channels * 8, num_slots_M=2,iters=3,resolutions=16)
        self.interFA1_1 = InterFA(in_channels=self.ft_chns[1])
        self.interFA2_1 = InterFA(in_channels=self.ft_chns[2])
        self.interFA3_1 = InterFA(in_channels=self.ft_chns[3])
        self.interFA1_2 = InterFA(in_channels=self.ft_chns[1])
        self.interFA2_2 = InterFA(in_channels=self.ft_chns[2])
        self.interFA3_2 = InterFA(in_channels=self.ft_chns[3])
    ...
    def forward(self, f0, f1, f2, f3, f4):
        ...
        f4_1 = self.ODE1(f4)
        temp3_1, f3_1 = self.interFA3_1(f3, f4_1)
        temp2_1, f2_1 = self.interFA2_1(f2, temp3_1)
        temp1_1, f1_1 = self.interFA1_1(f1, temp2_1)

        f4_2 = self.ODE2(f4)
        temp3_2, f3_2 = self.interFA3_2(f3, f4_2)
        temp2_2, f2_2 = self.interFA2_2(f2, temp3_2)
        temp1_2, f1_2 = self.interFA1_2(f1, temp2_2)
        return [f0, f1_1, f2_1, f3_1, f4_1], [f0, f1_2, f2_2, f3_2, f4_2]


class GCM_interFA_ODE_slot_channel4_scribble_thin(nn.Module):
    def __init__(self, in_channels, out_channels, params):
        super(GCM_interFA_ODE_slot_channel4_scribble_thin, self).__init__()
        self.ft_chns = params['feature_chns']


        self.interFA0_1 = InterFA(in_channels=int(in_channels/2))
        self.interFA1_1 = InterFA(in_channels=in_channels)
        self.interFA2_1 = InterFA(in_channels=in_channels*2)
        self.interFA3_1 = InterFA(in_channels=in_channels * 4)
        
        self.interFA0_2 = InterFA(in_channels=int(in_channels/2))
        self.interFA1_2 = InterFA(in_channels=in_channels)
        self.interFA2_2 = InterFA(in_channels=in_channels*2)
        self.interFA3_2 = InterFA(in_channels=in_channels * 4)
    ...
    def forward(self, f0, f1, f2, f3, f4):
        ...
        temp3_1, f3_1 = self.interFA3_1(f3, f4)
        temp2_1, f2_1 = self.interFA2_1(f2, temp3_1)
        temp1_1, f1_1 = self.interFA1_1(f1, temp2_1)
        temp0_1, f0_1 = self.interFA0_1(f0, temp1_1)


        temp2_2, f2_2 = self.interFA2_2(f2, temp3_2)
        temp1_2, f1_2 = self.interFA1_2(f1, temp2_2)
        temp0_2, f0_2 = self.interFA0_2(f0, temp1_2)
        
        return [f0_1, f1_1, f2_1, f3_1, f4], [f0_2, f1_2, f2_2, f3_2, f4]
