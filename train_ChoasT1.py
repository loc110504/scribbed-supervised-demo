...
from dataset.dataset_Choas import BaseDataSets, RandomGenerator
...
parser.add_argument('--exp', type=str,
                    default='Choas/InterFA_SA_Two', help='experiment_name')
...
parser.add_argument('--ca_iteration', type=int, default=1000)
...
def train(args, snapshot_path, savepath):
    ...
    dice_loss = pDLoss(num_classes, ignore_index=5)
    ...
    for epoch_num in iterator:
        for i_batch, sampled_batch in enumerate(trainloader):
            ...
            volume_batch, label_batch, pesudo_label_batch,conf,id = sampled_batch['image'].cuda(), sampled_batch['scribble'].cuda(), sampled_batch['pesudo_label'].cuda(),sampled_batch['conf'].cuda(),sampled_batch['idx']
...
######################################################################
            ...
            if iter_num>=args.ca_iteration:
                ...
                conf = F.normalize(conf.view(args.batch_size,4,-1), dim=2).view(args.batch_size,4,args.patch_size[0],args.patch_size[1])
                for i in range(batch_size):
                    size = 0
                    for k in range(1,(num_classes)):
                        if  (label_batch[i]==k).sum()==0:
                            continue
                        area = pesudo_label_batch[i]==k
                         
                        sam_conf[i,k][area] = conf[i,k-1][area] * outputs[i,k][area].mean()
                        conf_bg += conf[i,k-1]

                        sam_conf_1[i,k][area] = conf[i,k-1][area] * mainseg[i,k][area].mean()
                        conf_bg_1 += conf[i,k-1]

                        sam_conf_2[i,k][area] = conf[i,k-1][area] * auxseg[i,k][area].mean()
                        conf_bg_2 += conf[i,k-1]

                        print(np.unique(sam_conf[i,k].cpu().detach().numpy()))
                        size += 1
            ...
            logging.info(
                ...
                (iter_num, loss.item(), loss_ce.item(), loss_SCA.item(),loss_ssa.item(), alpha))
            ...
            if iter_num > 0 and iter_num % 200 == 0:
                ...
                dice_cls0,dice_cls1,dice_cls2,dice_cls3 = metric_list[0,0],metric_list[1,0],metric_list[2,0],metric_list[3,0]
                HD95_cls0,HD95_cls1,HD95_cls2,HD95_cls3 = metric_list[0,1],metric_list[1,1],metric_list[2,1],metric_list[3,1]
                writer.add_scalar('info/val_mean_dice', performance, iter_num)
                writer.add_scalar('info/val_mean_hd95', mean_hd95, iter_num)

                if performance > best_performance:
                    best_performance,best_hd95 = performance,mean_hd95
                    best_dice_0,best_dice_1,best_dice_2,best_dice_3 = dice_cls0,dice_cls1,dice_cls2,dice_cls3
                    best_hd95_0,best_hd95_1,best_hd95_2,best_hd95_3 = HD95_cls0,HD95_cls1,HD95_cls2,HD95_cls3
                ...
                logging.info('iteration %d : LIV_dice : %f LIV_hd95 : %f' % (iter_num, dice_cls0, HD95_cls0))
                logging.info('iteration %d : RK_dice : %f RK_hd95 : %f' % (iter_num, dice_cls1, HD95_cls1))
                logging.info('iteration %d : LK_dice : %f LK_hd95 : %f' % (iter_num, dice_cls2, HD95_cls2))
                logging.info('iteration %d : PSL_dice : %f PSL_hd95 : %f' % (iter_num, dice_cls2, HD95_cls2))
