...
from dataset.dataset_SegPC import BaseDataSets_SAM,RandomGenerator_SAM
...
def get_args_parser():
    ...
    parser.add_argument('--root_path', type=str, default='/home/cj/code/model_New/datasets/SegPC2021/SegPC_training_SAM_PL_iteration1')
    ...
    parser.add_argument('--exp', type=str, default='SAM_FineTune/iteration1_vith/cell')
    parser.add_argument('--iter', type=str, default='iteration1')
    
    parser.add_argument('--max_epochs', type=int, default=30, help='total epoch')
    parser.add_argument('--base_lr', type=float, default=1e-6, help='initial learning rate')
...
def main(opts, snapshot_path, savepath):
    ...
    ### Dataset & Dataloader ### 
    train_set = BaseDataSets_SAM(base_dir=opts.root_path, fold='fold1',transform=transforms.Compose(
        [RandomGenerator_SAM(opts.patch_size,split='train')]), split="train",pesudo_label = 'SAM_PL')
    

    val_set = BaseDataSets_SAM(base_dir=opts.root_path, fold='fold1', split="val", transform=transforms.Compose(
    ...
    diceloss = SAM_loss.pDLoss(3,ignore_index=3)
    ...
    for epoch in range(opts.max_epochs):
        ...
        train_loss = SAM_trainer.model_train_cell(
            model=sam,
        ...
        val_dice, val_HD95, val_iou = SAM_trainer.model_evaluate_cell(
            model=sam,
        ...
        print(f'epoch {epoch+1:02d}, dice_loss: {train_loss:.5f} \n')
